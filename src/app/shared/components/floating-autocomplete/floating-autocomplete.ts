...
@Component({
  selector: 'app-floating-autocomplete',
  standalone: true,
  template: `
    <div class="fi-group" [class.fi-group--error]="error">
      ...
      <!-- Native html datalist for autocompletion -->
      <datalist [id]="listId">
  ...
  providers: [{
    ...
    useExisting: forwardRef(() => FloatingAutocompleteComponent),
    multi: true,
  }]
})
export class FloatingAutocompleteComponent implements ControlValueAccessor {
  @Input() label = '';
  @Input() type = 'text';
  @Input() error = '';
  @Input() min?: string;
  ...
  inputId = 'fi-' + Math.random().toString(36).slice(2, 8);
  listId = 'list-' + this.inputId;
